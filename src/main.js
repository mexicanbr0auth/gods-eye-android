...
import { getGoogleMapsKeyWithFallback, getCesiumTokenWithFallback } from './androidBridge.js';
...
async function init() {
  ...
  try {
    ...
    // Set Cesium Ion token for World Terrain (com fallback Android)
    const cesiumToken = getCesiumTokenWithFallback(import.meta.env.CESIUM_ION_TOKEN);
    if (cesiumToken) {
      Cesium.Ion.defaultAccessToken = cesiumToken;
    }

    // Set Google Maps API key for 3D Tiles (Android SharedPreferences > localStorage > vite env)
    const googleApiKey = getGoogleMapsKeyWithFallback(import.meta.env.GOOGLE_MAPS_API_KEY);
    if (!googleApiKey) {
      const isAndroid = typeof window !== 'undefined' && (window.__IS_ANDROID__ || navigator.userAgent.includes('GodEyeAndroid'));
      if (isAndroid) {
        // No Android, mostra diálogo nativo em vez de crash
        loaderStatus.textContent = 'Aguardando chave do Google Maps...';
        const msg = document.createElement('div');
        msg.style.cssText = 'position:fixed;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#0a0e1a;color:#fff;z-index:9999;padding:24px;text-align:center;font-family:Inter,sans-serif';
        msg.innerHTML = `
          <h2 style="color:#00e5ff;margin-bottom:12px">Chave necessária</h2>
          <p style="max-width:480px;line-height:1.5;margin-bottom:16px">
            O globo 3D precisa de <b>GOOGLE_MAPS_API_KEY</b>.<br>
            Toque em <b>Configurar chaves</b> no menu (⋮) e cole sua chave do Google Cloud (Map Tiles API).
          </p>
          <button id="android-config-keys" style="padding:12px 24px;background:#00e5ff;color:#000;border:none;border-radius:8px;font-weight:600;cursor:pointer">Configurar chaves</button>
          <p style="margin-top:16px;font-size:12px;opacity:.6">Você também pode definir CESIUM_ION_TOKEN para Bing opcional.</p>
        `;
        document.body.appendChild(msg);
        document.getElementById('android-config-keys')?.addEventListener('click', () => {
          try { window.Android?.configureKeys?.(); } catch {}
        });
        // Não lança; mantém app vivo para o usuário configurar
        console.warn('[GodEye Android] GOOGLE_MAPS_API_KEY ausente — aguardando configuração.');
        return;
      }
